...
public class LoginFrame extends JFrame {
    ...
    public LoginFrame(Connector base, Frame parent){
        super("Logining");
    ...
    private void createAcceptButton (){
        ...
        acceptButton.addActionListener(ae -> {
            ...
            User user;
            if(base.connectUser(login, password)){
                user = base.getUserByName(login);
                //System.out.println(user.getName() + " " + user.getId());

                correctLabel.setText("С возвращением!");
                correctLabel.setForeground(new Color(25, 175, 25));

                parent.setCurrentUser(user);
            } else {
                correctLabel.setText("Неверный логин и/или пароль");
